...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    S = self.dropout(nn.Softmax(dim=-1)(S))
    
    V = torch.matmul(S, value)  # [bs, num_attention_heads, seq_len, attention_head_size]
    # print(V.shape) # torch.Size([2, 12, 8, 64])
    V = V.transpose(1, 2).contiguous()
    V = V.view(*V.shape[:2],-1) # [bs, seq_len, num_attention_heads * attention_head_size]
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    hidden_states = ln_layer(hidden_states + input)
...
class BertModel(BertPreTrainedModel):
  ...
  def forward(self, input_ids = None, attention_mask = None, embedding_output = None):
    """
